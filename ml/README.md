...
```
lambdas/etl/handler.py        ETL:清理 + 特徵工程(S3 觸發;run_etl 為可本地測試的純轉換)
ml/training/
...
└── __init__.py
ml/examples/
├── submission.csv            範例:102 個預測(提交格式)
└── fleet_data.json           範例:前端用完整資料(全船隊逐日 + 養護 + 預測 + 驗證)
...
clean  = handler.run_etl(voyage, maint)          # 20,938 列,102 PREDICT

# 2) 訓練 + 預測
clean = model.add_derived(clean)                 # 加 log_v3d23、cast 類別欄
mdl   = model.train(clean)                       # 物理殘差 hybrid
sub   = model.predict_submission(mdl, clean)     # 102 列: ship_id/day/fuel_type/predicted_value

# 3) 驗證(maintenance 需對齊到 NOON_UTC day-index)
mt  = handler.align_event_day(maint)
cv  = validation.event_holdout_cv(clean, mt)
print(validation.mape(cv.truth, cv.pred))        # ~4.2%
```

雲端路徑:S3 上傳原始 CSV → 觸發 ETL Lambda(`lambda_handler`)→ 寫 processed Parquet → 模型讀 `model.load_processed(s3_uri)`。部署選型見 `docs/ml-eda-and-decisions.md`(採 Lambda container image)。
...
| 養護 join | `join_maintenance` | 先 `align_event_day`(calendar `event_date` → day-index,epoch ≈ 2021-01-01),再算距上次船殼清洗/螺旋槳拋光/進塢天數、上次事件類型/螺旋槳狀態、硬質汙損旗標、`fouling_severity_score`。只用 `event_day ≤ 當日` 的事件(無未來洩漏) |
...
**特徵(`config.py`,單一真實來源)**
- 數值:`log_v3d23`(物理核心)、STW、RPM、螺旋槳轉速、排水量、吃水、trim、載貨、風/浪/湧、海溫、水深、洋流代理 `DIFF_STW_SOG_SLIP`、滑差 `FULL_SPD_STW_SLIP`(汙損訊號)、養護時鐘 `days_since_*`、`last_event_had_hard_fouling`、`fouling_severity_score`、全速時數。
...
由 `model.predict_submission(mdl, df)` 產生。
